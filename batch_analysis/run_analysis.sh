...
echo -e "\n${YELLOW}[1.5/5] Levantando servicios de Hadoop...${NC}"
docker-compose up -d hadoop-namenode hadoop-datanode
echo "Esperando 60 segundos para que HDFS esté listo..."
sleep 60

if ! docker-compose ps hadoop-namenode | grep -q "Up"; then
    echo "Error: hadoop-namenode no está corriendo"
    exit 1
fi

echo -e "${GREEN}Hadoop está listo${NC}"
...
# Usar rutas absolutas con realpath para manejar caracteres especiales en el path
PROJECT_DIR="$(realpath "$(dirname "${BASH_SOURCE[0]}")/..")"
docker-compose run --rm -v "${PROJECT_DIR}/batch_analysis:/app" -v "${PROJECT_DIR}/batch_analysis/results:/results" \
