...
docker-compose exec -T hadoop-namenode bash -c "export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop && hdfs dfs -mkdir -p /input && hdfs dfs -put /data/yahoo_answers.txt /input/ && hdfs dfs -put /data/llm_answers.txt /input/"

# 5. Ejecutar análisis Pig
docker-compose exec -T hadoop-namenode bash -c "export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop && export PIG_CLASSPATH=/opt/hadoop/etc/hadoop && pig -x mapreduce /pig_scripts/wordcount_yahoo.pig"
docker-compose exec -T hadoop-namenode bash -c "export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop && export PIG_CLASSPATH=/opt/hadoop/etc/hadoop && pig -x mapreduce /pig_scripts/wordcount_llm.pig"
docker-compose exec -T hadoop-namenode bash -c "export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop && export PIG_CLASSPATH=/opt/hadoop/etc/hadoop && pig -x mapreduce /pig_scripts/comparative_analysis.pig"

# 6. Descargar resultados
docker-compose exec -T hadoop-namenode bash -c "export HADOOP_CONF_DIR=/opt/hadoop/etc/hadoop && hdfs dfs -get /output/* /results/"
